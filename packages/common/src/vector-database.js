class QdrantVectorDatabase {
  ...
  async ensureCollection(size) {
    const response = await this.fetch(`${this.baseUrl}/collections/${this.collectionName}`, { headers: this.headers() });
    if (response.ok) {
  ...
  async resetCollection() {
    let response;
    try { response = await this.fetch(`${this.baseUrl}/collections/${this.collectionName}`, { method: 'DELETE', headers: this.headers() }); }
  ...
  async deleteByFilter(filter) {
    const existing = await this.fetch(`${this.baseUrl}/collections/${this.collectionName}`, { headers: this.headers() });
